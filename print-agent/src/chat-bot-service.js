...
class ChatBotService extends EventEmitter {
  /**
   * @param {string} sessionsFile  - caminho para bot-sessions.json em userData
   * @param {() => object} getConfig - retorna configurações atuais do store
  ...
  _saveSessions() {
    try {
      ...
      console.error('[ChatBot] Erro ao salvar sessoes:', err);
    }
  }
